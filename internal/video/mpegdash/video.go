...
// ServeMpegDash starts a static file server and stream video from the Raspberry Pi camera module using the
// MPEG DASH format.
//
// This is a blocking operation.
func ServeMpegDash(port int, directory string, options Options) {
	...
	// Stream video
	go func() {
		streamMpegDash(directory, options)
		wg.Done()
	}()
...
func streamMpegDash(directory string, options Options) {
	// Pipe video stream from raspivid into ffmpeg
	raspivid := raspivid.Stream(toRaspividOptions(options))
	ffmpeg := mpegdash.MpegDash(raspivid.Video, directory, toMpegDashOptions(options))
...
func toMpegDashOptions(options Options) mpegdash.Options {
	return mpegdash.Options{
		Fps:          options.Fps,
